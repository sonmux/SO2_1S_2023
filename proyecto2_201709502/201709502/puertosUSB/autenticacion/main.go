...
func main() {
	...
	err = notify.Watch("/media/vboxuser/Ubuntu 20_04_4 LTS amd64", c, notify.Create)
	if err != nil {
		log.Fatal(err)
	}
	defer notify.Stop(c)
	...
	for {
		...
		case ei := <-c:
			...
			_, err := file.WriteString(fmt.Sprintf("archivo copiado a la USB: %s\n", filename))
			if err != nil {
				log.Fatal(err)
			}
			// Imprime el nombre del archivo en la consola
			fmt.Printf("Archivo copiado: %s\n", filename)
		}
	}
}
